...
#define DECLARE_EVENT(name, owner, ...) \
	...
	public:
		Delegate() = default;
		WHGFX_DEFAULT_COPYABLE_MOVABLE(Delegate)
		~Delegate() = default;
	...
	public:
		...
		WHGFX_DEFAULT_COPYABLE(DelegateHandle)
	...
	public:
		...
		WHGFX_DEFAULT_COPYABLE_MOVABLE(MultiCastDelegate)
			~MultiCastDelegate() = default;

		WHGFX_MAYBE_UNUSED DelegateHandle Add(DelegateType&& handler)
		{
			delegate_array.emplace_back(DelegateHandle(0), std::forward<DelegateType>(handler));
			return delegate_array.back().first;
		}
		WHGFX_MAYBE_UNUSED DelegateHandle Add(DelegateType const& handler)
		{
		...
		template<typename F>
		WHGFX_MAYBE_UNUSED DelegateHandle AddLambda(F&& lambda)
		{
			return Add(DelegateType::CreateLambda(std::forward<F>(lambda)));
		}

		template<typename R>
		WHGFX_MAYBE_UNUSED DelegateHandle AddStatic(R(*pf)(Args...))
		{
			return Add(DelegateType::CreateStatic(pf));
		}

		template<typename T>
		WHGFX_MAYBE_UNUSED DelegateHandle AddMember(void(T::* mem_pfn)(Args...), T& instance)
		{
			return Add(DelegateType::CreateMember(mem_pfn, instance));
		}

		WHGFX_MAYBE_UNUSED bool Remove(DelegateHandle& handle)
		{
			if (handle.IsValid())
			{
